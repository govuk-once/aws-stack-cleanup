...
describe('Stack Manager tests', () => {
  test('Should list all active stacks', async () => {
    const manager = new StackManager(new MockCloudFormationClient());

    const stacks = await manager.getStacks();
  ...
  test('Should place be able to list stack in deletion order', async () => {
    ...
    const deletionOrder = await manager.getDeletionOrder(stacks);
