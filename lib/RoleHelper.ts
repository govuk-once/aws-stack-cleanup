...
export class RoleHelper {
    ...
    addDynamoOperationPermissionsToLambda(props: IRoleHelperProps): iam.Role {
        if (!props.table)
            throw "table must be supplied to add s3 role to lambda";
    ...
    addS3OperationPermissionsToLambda(props: IRoleHelperProps): iam.Role {
        if (!props.bucket)
            throw "bucket must be supplied to add s3 role to lambda";
        const role = this.findOrCreateRole(props);
        const { bucketActions, objectActions } = this.createS3Operations(props.operations);
    ...
    protected findOrCreateRole(props: IRoleHelperProps): iam.Role {
        ...
        if (roleCandidate)
            return roleCandidate as iam.Role;
        return new iam.Role(this.scope, this.namingProvider.getResourceId(props.id) ?? "noId", {
