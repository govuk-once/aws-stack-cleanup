    ...
    hostedZoneName(): string {
        if (!this.hostedZoneNamecache) {
            this.hostedZoneNamecache =
                cdk.aws_ssm.StringParameter.fromStringParameterName(this.scope, `HostedZoneName`, "/infra/dns/hostedzonename");
        }
    ...
    hostedZoneId(): string {
        if (!this.hostedZoneIdCache) {
            this.hostedZoneIdCache =
                cdk.aws_ssm.StringParameter.fromStringParameterName(this.scope, `HostedZoneId`, "/infra/dns/hostedzoneid");
        }
    ...
    acmCertArn(): cdk.aws_ssm.IStringParameter {
        if (!this.acmCertArnCache) {
            this.acmCertArnCache =
                cdk.aws_ssm.StringParameter.fromStringParameterName(this.scope, "acmCertArn", "/infra/acm/certificatearnregional");
        }
    ...
    zone(): cdk.aws_route53.IHostedZone {
        if (!this.hostedZoneCache) {
            this.hostedZoneCache =
                cdk.aws_route53.HostedZone.fromHostedZoneAttributes(this.scope, "HostedZone", {
    ...
    certificate(): ICertificate {
        if (!this.certificateCache) {
            this.certificateCache =
                cdk.aws_certificatemanager.Certificate.fromCertificateArn(this.scope, "AcmCertificate", this.acmCertArn().stringValue);
        }
